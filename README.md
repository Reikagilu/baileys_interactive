...
| `WEBHOOK_INCLUDE_INCOMING_MEDIA_BASE64` | Inclui `base64` em mídia recebida (`audio`, `image`, `sticker`, `document`) no evento `messages.upsert` e no chat | false |
| `WEBHOOK_INCLUDE_INCOMING_VIDEO_BASE64` | Inclui `base64` em `video` recebido (pode gerar payload grande) | false |
