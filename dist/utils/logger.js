...
// ── Core formatter ──────────────────────────────────────────────────────────
function format(level, module, instance, message) {
    const ts = c(DIM, `[${timestamp()}]`);
    // Module badge
    let moduleBadge;
    switch (level) {
        case 'error':
            moduleBadge = c(`${BOLD}${BRIGHT_RED}`, `[${module}]`);
            break;
        case 'warn':
            moduleBadge = c(BRIGHT_YELLOW, `[${module}]`);
            break;
        case 'success':
            moduleBadge = c(GREEN, `[${module}]`);
            break;
        case 'debug':
            moduleBadge = c(DIM, `[${module}]`);
            break;
        default:
            moduleBadge = c(BRIGHT_CYAN, `[${module}]`);
            break;
    }
    ...
    switch (level) {
        case 'error':
            msg = c(`${BOLD}${RED}`, message);
            break;
        case 'warn':
            msg = c(YELLOW, message);
            break;
        case 'success':
            msg = c(GREEN, message);
            break;
...
function makeLogger(module, instance) {
    ...
    return {
        ...
        child(inst) { return makeLogger(module, inst); },
    };
}
