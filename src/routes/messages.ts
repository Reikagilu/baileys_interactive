...
function extractTypingSeed(body: Record<string, unknown>, content: unknown): string {
...
function parseDeliveryTimeoutMs(raw: unknown): number {
    const value = Number(raw);
    if (!Number.isFinite(value)) return 15_000;
    return Math.max(1_000, Math.min(60_000, Math.round(value)));
}

function parseRequireDelivery(raw: unknown): boolean {
...
function buildN8nSendContext(body: Record<string, unknown>, validation: {
    input: string;
...
async function sendMessageWithTyping(
    ctx: ReturnType<typeof getInstance>,
    jid: string,
    ...
    if (ctx && typingMs && typeof ctx.sock.sendPresenceUpdate === 'function') {
        ...
        try {
            await (ctx.sock as { presenceSubscribe?: (jid: string) => Promise<void> }).presenceSubscribe?.(jid);
        } catch {
...
function validateInstance(instanceName: string, res: Response): ReturnType<typeof getInstance> | null {
...
function resolveInstanceName(rawInstance: unknown, res: Response): string | null {
    const instance = normalizeInstanceName(rawInstance as string, 'main');
    if (!instance) {
...
function parseInteractiveCtas(rawCtas: unknown): CtaButton[] {
...
async function sendBasicMessage(
    req: Request,
    res: Response,
    contentFactory: (body: Record<string, unknown>) => Record<string, unknown> | null,
    ...
    if (!validation.ok) {
        ...
        sendError(
            res,
            statusCode,
            validation.error ?? 'recipient_validation_failed',
            validation.message,
            buildN8nSendContext(body, validation),
        );
        return;
    }
    ...
    if (idempotencyKey) {
        ...
        if (!acquireIdempotencyLock(idempotencyKey, idempotencyScope)) {
            sendError(
                ...
                'Outro request com a mesma idempotency-key está sendo processado.',
            );
            return;
        }
    }
    ...
    try {
        ...
        const sendTimeout = new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('send_timeout')), 60_000),
        );
        try {
        ...
        const sentKey = (sent as { key?: { id?: string } } | undefined)?.key;
        ...
        if (!messageId) {
            sendError(
                ...
                502,
                'send_no_ack',
                'O Baileys nao retornou messageId. O WhatsApp nao confirmou nem o aceite inicial da mensagem.',
                buildN8nSendContext(body, validation),
            );
            return;
        }
        ...
        if (delivery.snapshot.state === 'failed') {
            sendError(
                ...
                delivery.snapshot.description,
                { ...contextDetails, delivery: deliveryDetails },
            );
            return;
        }
        if (requireDelivery && (delivery.timedOut || delivery.snapshot.state === 'pending' || delivery.snapshot.state === 'server_ack')) {
            sendError(
                ...
                'A mensagem foi aceita pela API, mas nao houve confirmacao de entrega ao aparelho do destinatario dentro do tempo limite.',
                { ...contextDetails, delivery: deliveryDetails },
            );
            return;
        }
        ...
        emitWebhookEvent(
            'messages.upsert',
            { source: 'api', direction: 'outbound', instance, to: jid, messageId, content },
            instance,
        );
...
router.post('/media', async (req: Request, res: Response) => {
    ...
    const to = String(body.to ?? '').trim();
    const jid = toJid(to);
    if (!jid) return void sendError(res, 400, 'invalid_phone');
    const ctx = validateInstance(instance, res);
    if (!ctx) return;
    ...
    try {
        ...
        const sendTimeout = new Promise<never>((_, reject) =>
            setTimeout(() => reject(new Error('send_timeout')), 90_000),
        );
        let result: { ok: boolean; id?: string; error?: string } | undefined;
        try {
...
router.post('/forward', (req: Request, res: Response) =>
    sendBasicMessage(req, res, (body) => {
        ...
        if (forwardedContent && typeof forwardedContent === 'object') {
            ...
            for (const [k, v] of Object.entries(forwardedContent as Record<string, unknown>)) {
                if (FORWARD_ALLOWED_KEYS.has(k)) safe[k] = v;
            }
...
router.post('/send_menu', async (req: Request, res: Response) => {
    ...
    const lines = options.map(
        (opt, idx) => `${idx + 1}. ${opt.text}${opt.description ? ` — ${opt.description}` : ''}`,
    );
    const menuText = [
    ...
    emitWebhookEvent(
        'messages.upsert',
        { source: 'api', direction: 'outbound', instance, to: jid, messageId: sentKey?.id, content: menuContent },
        instance,
    );
...
router.post('/send_buttons_helpers', async (req: Request, res: Response) => {
    ...
    const jid = toJid(String(body.to ?? ''));
    if (!jid) return void sendError(res, 400, 'invalid_phone');
    const buttons = Array.isArray(body.buttons) ? (body.buttons as Record<string, unknown>[]) : [];
    ...
    emitWebhookEvent(
        'messages.upsert',
        { source: 'api', direction: 'outbound', instance, to: jid, messageId: sentKey?.id, content },
        instance,
    );
...
router.post('/send_list_helpers', async (req: Request, res: Response) => {
    ...
    const jid = toJid(String(body.to ?? ''));
    if (!jid) return void sendError(res, 400, 'invalid_phone');
    const sections = Array.isArray(body.sections) ? (body.sections as Record<string, unknown>[]) : [];
    ...
    for (const section of sections) {
        ...
        if ((section.rows as unknown[]).length > config.limits.maxListRowsPerSection) {
...
router.post('/send_poll', async (req: Request, res: Response) => {
    ...
    const options = Array.isArray(body.options)
        ? (body.options as unknown[]).filter((s): s is string => typeof s === 'string' && Boolean(s.trim()))
        : [];
...
router.post('/send_carousel_helpers', async (req: Request, res: Response) => {
    ...
    const jid = toJid(String(body.to ?? ''));
    if (!jid) return void sendError(res, 400, 'invalid_phone');
    const cards = Array.isArray(body.cards) ? (body.cards as Record<string, unknown>[]) : [];
    ...
    for (const card of cards) {
        const cardButtons = Array.isArray(card.buttons) ? (card.buttons as unknown[]) : [];
    ...
    const carouselCards = cards.map((card) => ({
        ...
        buttons: (Array.isArray(card.buttons) ? (card.buttons as Record<string, unknown>[]) : []).map((button) => ({
            name: 'quick_reply',
