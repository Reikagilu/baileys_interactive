...
const _inFlight = new Map<string, number>(); // storeKey → expiresAt
...
function pruneAll(): void {
    ...
    for (const [k, expiresAt] of _inFlight) {
        if (expiresAt <= now) _inFlight.delete(k);
    }
}
 ...
 */
export function acquireIdempotencyLock(key: string, scope: string): boolean {
    if (!config.idempotency.enabled || !key) return true;
    ...
    if (inFlightExpiry !== undefined) {
        if (inFlightExpiry > Date.now()) return false; // lock ativo
...
export function releaseIdempotencyLock(key: string, scope: string): void {
    if (!config.idempotency.enabled || !key) return;
