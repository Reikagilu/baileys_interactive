...
interface WebhookRow {
    ...
    instance: string | null; enabled: number; secret: string | null;
    created_at: number; updated_at: number;
}

export interface Webhook {
    id: string; name: string; url: string; events: string[];
    instance?: string; enabled: boolean; secret?: string;
    createdAt: number; updatedAt: number;
}

export interface WebhookDelivery {
    id: string; webhookId: string; webhookName: string; webhookUrl: string;
    event: string; instance?: string; status: string;
    attemptCount: number; maxAttempts: number;
    nextAttemptAt: number; lastAttemptAt?: number; deliveredAt?: number;
    lastError?: string; responseStatus?: number;
    lockOwner?: string; lockExpiresAt?: number;
    payload: unknown; createdAt: number; updatedAt: number;
}

export interface WebhookMetrics {
    webhooksTotal: number; webhooksEnabled: number;
    deliveriesPending: number; deliveriesProcessing: number;
    deliveriesDelivered: number; deliveriesFailed: number;
    deliveriesTotal: number; oldestPendingAgeSeconds: number;
}
...
function openDatabase(dbPath: string): DatabaseSync {
    ...
    try { db.exec('PRAGMA synchronous = NORMAL;'); } catch { /* optional */ }
    return db;
}
...
function toWebhook(row: Record<string, unknown> | WebhookRow): Webhook {
    return {
        id: String(row.id),
...
function toEventEnvelope(event: string, payload: unknown, instance: string | undefined) {
    return {
        id: randomUUID(),
        event,
...
function scheduleCleanupTimer(): void {
    ...
    _cleanupTimer = setInterval(() => {
        try { runCleanupDeliveryHistory(); } catch { /* ignore */ }
    }, intervalMs);
    _cleanupTimer.unref?.();
}

function runCleanupDeliveryHistory(): void {
    const db = getDb();
    const countRow = db.prepare('SELECT COUNT(*) AS c FROM webhook_deliveries').get() as {c: number};
...
export function createWebhook(input: {
    ...
    db.prepare(`INSERT INTO webhooks (id, name, url, events, instance, enabled, secret, created_at, updated_at)
     VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(
        id, input.name, input.url, JSON.stringify(events),
        input.instance ?? null, (input.enabled ?? true) ? 1 : 0,
        input.secret ?? null, now, now,
    );
...
export function updateWebhook(id: string, update: Partial<Pick<Webhook, 'name' | 'url' | 'events' | 'instance' | 'enabled' | 'secret'>>): Webhook | null {
    const db = getDb();
    ...
    db.prepare(`UPDATE webhooks
     ...
     WHERE id = ?`).run(
        next.name, next.url, JSON.stringify(next.events),
        next.instance ?? null, next.enabled ? 1 : 0,
        next.secret ?? null, now, id,
    );
    invalidateWebhooksCache();
    return next;
}
...
function enqueueDelivery(db: DatabaseSync, webhook: Webhook, event: string, payload: unknown, instance?: string): void {
    ...
    db.prepare(`INSERT INTO webhook_deliveries (
      ...
      max_attempts, next_attempt_at, payload, created_at, updated_at
        ...
        randomUUID(), webhook.id, webhook.name, webhook.url, event,
        instance ?? null, 'pending', 0, config.webhooks.maxAttempts,
        now, JSON.stringify(toEventEnvelope(event, payload, instance)), now, now,
    );
}

export function emitWebhookEvent(event: string, payload: unknown, instance?: string): { queued: number } {
...
export function enqueueWebhookTestDelivery(webhookId: string, event: string, payload: unknown): { queued: number; reason?: string } {
    const webhook = getWebhook(webhookId);
    if (!webhook) return { queued: 0, reason: 'webhook_not_found' };
    if (!webhook.enabled) return { queued: 0, reason: 'webhook_disabled' };
    const db = getDb();
...
export function listWebhookDeliveries(filters?: { webhookId?: string; status?: string; limit?: number }): WebhookDelivery[] {
    const db = getDb();
    const conditions: string[] = [];
    const params: unknown[] = [];
    if (filters?.webhookId) { conditions.push('webhook_id = ?'); params.push(filters.webhookId); }
    if (filters?.status) { conditions.push('status = ?'); params.push(filters.status); }
    const where = conditions.length ? `WHERE ${conditions.join(' AND ')}` : '';
    const limit = Math.min(Math.max(filters?.limit ?? 50, 1), 500);
    const rows = db.prepare(`SELECT * FROM webhook_deliveries ${where} ORDER BY created_at DESC LIMIT ?`).all(...(params as Parameters<ReturnType<DatabaseSync['prepare']>['all']>), limit) as Record<string, unknown>[];
...
export function claimDueDeliveries(batchSize: number, workerId: string, lockMs: number): WebhookDelivery[] {
    ...
    try {
        const rows = db.prepare(`SELECT id FROM webhook_deliveries
         ...
         LIMIT ?`).all(now, now, batchSize) as Array<{id: string}>;

        if (!rows.length) { db.exec('COMMIT'); return []; }
       ...
       WHERE id IN (${placeholders})`).run(workerId, lockExpiresAt, now, ...ids);
        ...
        const claimedRows = db.prepare(`SELECT * FROM webhook_deliveries WHERE id IN (${placeholders}) ORDER BY next_attempt_at ASC`).all(...ids) as Record<string, unknown>[];
        db.exec('COMMIT');
...
export function markDeliverySuccess(id: string, responseStatus: number, workerId: string): void {
    const now = Date.now();
...
export function markDeliveryRetry(id: string, lastError: string, responseStatus: number | null, attemptCount: number, workerId: string): void {
...
export function markDeliveryFailed(id: string, lastError: string, responseStatus: number | null, workerId: string): void {
    const now = Date.now();
    getDb().prepare(`UPDATE webhook_deliveries
     SET status = 'failed',
         last_error = ?,
...
export function renewDeliveryLock(id: string, workerId: string, lockMs: number): boolean {
    const now = Date.now();
...
export function getWebhookMetrics(): WebhookMetrics {
    const db = getDb();
    const hooksRow = db.prepare('SELECT COUNT(*) AS total, SUM(CASE WHEN enabled = 1 THEN 1 ELSE 0 END) AS enabled FROM webhooks').get() as {total: number; enabled: number};
    const statusRows = db.prepare('SELECT status, COUNT(*) AS count FROM webhook_deliveries GROUP BY status').all() as Array<{status: string; count: number}>;
    const oldestPendingRow = db.prepare("SELECT created_at FROM webhook_deliveries WHERE status IN ('pending','processing') ORDER BY created_at ASC LIMIT 1").get() as {created_at: number} | undefined;
    const counts = { pending: 0, processing: 0, delivered: 0, failed: 0 };
    for (const row of statusRows) {
        const status = String(row.status);
        const count = Number(row.count ?? 0);
        if (status === 'pending') counts.pending = count;
        else if (status === 'processing') counts.processing = count;
        else if (status === 'delivered') counts.delivered = count;
...
export function loadWebhookForDelivery(id: string): Webhook | null {
