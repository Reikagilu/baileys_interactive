...
const requestBody = (schema, required = true) => ({ required, content: jsonContent(schema) });
...
function add(method, path, tag, summary, options = {}) {
    ...
    if (options.body)
        operation.requestBody = requestBody(options.body, options.bodyRequired ?? true);
    paths[path] ??= {};
...
const sendSchema = (required, properties) => ({
    ...sendBase,
    required: ['to', ...required],
    properties: { ...sendBase.properties, ...properties },
});
...
for (const action of ['read', 'archive', 'unarchive', 'pin', 'unpin', 'mute', 'unmute']) {
    const properties = { instance: { type: 'string', default: 'main' } };
    if (action === 'read')
        properties.messageIds = { type: 'array', maxItems: 100, items: { type: 'string' } };
    if (action === 'mute')
        properties.durationSeconds = { type: 'integer', minimum: 1, maximum: 31536000 };
...
export const openApiSpec = {
    ...
    components: {
        ...
        schemas: {
            ...
            GeneralSettings: { type: 'object', properties: { rejectCalls: { type: 'boolean' }, ignoreGroups: { type: 'boolean' }, alwaysOnline: { type: 'boolean' }, autoReadMessages: { type: 'boolean' }, syncFullHistory: { type: 'boolean' }, readStatus: { type: 'boolean' }, importContacts: { type: 'boolean' } } },
