...
function isPrivateIPv4(hostname) {
    ...
    if (a === 192 && b === 0 && segments[2] === 2)
        return true; // TEST-NET-1
    if (a === 198 && (b === 51 || b === 52) && segments[2] === 100)
        return true; // TEST-NET-2
    if (a === 203 && b === 0 && segments[2] === 113)
        return true; // TEST-NET-3
    if (a >= 224 && a <= 239)
        return true; // multicast
    if (a >= 240)
        return true; // reserved + broadcast
    return false;
}
function isPrivateIPv6(hostname) {
    ...
    if (normalized === '::' || normalized === '::1')
        return true;
    if (normalized === '0:0:0:0:0:0:0:1' || normalized === '0:0:0:0:0:0:0:0')
        return true;
...
export function validateOutboundUrl(input, options = {}) {
 ...
 */
export async function validateOutboundUrlAsync(input, options = {}) {
    const sync = validateOutboundUrl(input, options);
    if (!sync.ok)
        return sync;
    if (options.allowPrivateNetwork || !options.resolveDns)
        return sync;
