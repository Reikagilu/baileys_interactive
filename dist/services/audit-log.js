...
export function writeAuditEvent(req, res, input) {
    try {
        ...
        if (_ringBuffer.length > _maxInMemory)
            _ringBuffer.shift();
        // Persist (best-effort; never throw to caller)
        try {
            stmt('audit.insert', `INSERT INTO audit_events (ts, request_id, action, target, outcome, actor_key_id, actor_scopes, req_method, req_path, req_ip, details_json, created_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`).run(event.ts, event.requestId ?? null, event.action, event.target ?? null, event.outcome, event.actor.keyId, JSON.stringify(event.actor.scopes), event.request.method, event.request.path, event.request.ip ?? null, event.details !== undefined ? JSON.stringify(event.details) : null, now);
        }
        catch (err) {
...
export function listRecentAuditEvents(limit = 100) {
    ...
    try {
        const rows = stmt('audit.list', `SELECT ts, request_id, action, target, outcome, actor_key_id, actor_scopes,
...
/** Used by tests. */
export function _clearAuditLogForTests() {
