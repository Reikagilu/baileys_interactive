...
const _creatingInstances = new Set<string>();
...
function hasIncompleteSavedPairingAuth(name: string): boolean {
    const authPath = path.resolve(process.cwd(), config.authFolder, name, 'creds.json');
    try {
        if (!fs.existsSync(authPath)) return false;
        const creds = JSON.parse(fs.readFileSync(authPath, 'utf8')) as Record<string, unknown>;
        const registered = creds?.registered === true;
        const hasPersistedIdentity = Boolean(
            String((creds?.me as Record<string, unknown>)?.id ?? '').trim() ||
            String((creds?.me as Record<string, unknown>)?.lid ?? '').trim(),
        );
        const hasCompletedPairingState =
            Boolean(creds?.account) ||
            (Array.isArray(creds?.signalIdentities) && (creds.signalIdentities as unknown[]).length > 0);
...
router.param('name', (req: Request, res: Response, next, rawName: string) => {
...
router.post('/', async (req: Request, res: Response) => {
    try {
        ...
        let result: Awaited<ReturnType<typeof createInstance>>;
        try {
        ...
        if (!result.ok) {
            if (result.error === 'auth_corrupt_requires_manual_repair') {
                return sendError(
                    ...
                    409,
                    result.error,
                    'A autenticacao salva desta instancia esta corrompida e precisa de reparo manual antes de conectar novamente.',
                    { instance: name },
                );
            }
            ...
            if (!created.ok) {
                if (created.error === 'auth_corrupt_requires_manual_repair') {
                    return sendError(
                        ...
                        409,
                        created.error,
                        'A autenticacao salva desta instancia esta corrompida e precisa de reparo manual antes de gerar pairing code.',
                        { instance: name },
                    );
                }
 ...
 */
router.post('/:name/repair-sessions', async (req: Request, res: Response) => {
    const { name } = req.params;
...
router.post('/:name/chats/:jid/sync-history', async (req: Request, res: Response) => {
    ...
    const result = await syncInstanceChatHistory(name, decodedJid, {
        ...
        fetchCount: body.fetchCount as number | undefined,
    });
    if (!result.ok) {
        const statusCode =
...
router.patch('/:name/settings/general', async (req: Request, res: Response) => {
    ...
    const updated = updateInstanceGeneral(name, {
        rejectCalls: body.rejectCalls as boolean | undefined,
        ignoreGroups: body.ignoreGroups as boolean | undefined,
        alwaysOnline: body.alwaysOnline as boolean | undefined,
    ...
    const syncRestartError = undefined;
...
const ALLOWED_PROXY_PROTOCOLS = ['http', 'https', 'socks4', 'socks5'] as const;
type AllowedProxyProtocol = (typeof ALLOWED_PROXY_PROTOCOLS)[number];
...
router.patch('/:name/settings/proxy', (req: Request, res: Response) => {
    ...
    if (body.protocol !== undefined) {
        ...
        if (!ALLOWED_PROXY_PROTOCOLS.includes(proto as AllowedProxyProtocol)) {
    ...
    const updated = updateInstanceProxy(name, {
        ...
        port: body.port as string | undefined,
...
router.patch('/:name/events', (req: Request, res: Response) => {
    ...
    const updated = updateInstanceEvents(name, {
        ...
        toggles: body.toggles as Record<string, boolean> | undefined,
    });
    writeAuditEvent(req, res, {
...
router.post('/:name/events/test', async (req: Request, res: Response) => {
    ...
    if (!(INSTANCE_EVENT_NAMES as readonly string[]).includes(eventName)) {
        return sendError(res, 400, 'invalid_event_name');
    }
    const testResult = await emitInstanceEvent(
        name,
        eventName as Parameters<typeof emitInstanceEvent>[1],
        { source: 'manual_test' },
        { ignoreToggle: true },
    );
    if (!testResult.ok) {
