...
import {
  persistChatwootOriginated,
  isChatwootOriginatedPersisted,
} from './chatwoot-sync-store.js';

/** msgId → expireTimestamp */
const chatwootOriginatedIds = new Map<string, number>();
...
function pruneExpired(now: number): void {
  for (const [id, exp] of chatwootOriginatedIds) {
    if (exp < now) chatwootOriginatedIds.delete(id);
  }
}

export function markChatwootOriginated(msgId: string): void {
  if (!msgId) return;
  const now = Date.now();
...
export function isChatwootOriginated(msgId: string): boolean {
  if (!msgId) return false;
  const exp = chatwootOriginatedIds.get(msgId);
  if (exp && exp >= Date.now()) return true;
  if (exp && exp < Date.now()) chatwootOriginatedIds.delete(msgId);
