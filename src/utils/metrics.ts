...
export interface InstanceMetrics {
  opencode_peer_self_fix: number;
  messages_upsert: number;
  last_message_at: string | null;
  last_opencode_peer_self_fix_at: string | null;
  last_connected_at: string | null;
  connection_close_codes: Record<number, number>;
...
function emptyInstance(): InstanceMetrics {
  return {
...
function ensureInstance(name: string): InstanceMetrics {
...
export function recordOpencodePeerSelfFix(name: string): void {
  const m = ensureInstance(name);
  m.opencode_peer_self_fix += 1;
  m.last_opencode_peer_self_fix_at = new Date().toISOString();
}

export function recordMessagesUpsert(name: string, count: number = 1): void {
  if (count <= 0) return;
  const m = ensureInstance(name);
...
export function recordConnected(name: string): void {
  ensureInstance(name).last_connected_at = new Date().toISOString();
}

export function recordConnectionClose(name: string, code: number | null): void {
  if (code == null) return;
  const m = ensureInstance(name);
  m.connection_close_codes[code] = (m.connection_close_codes[code] ?? 0) + 1;
}

export function recordReconnectAttempt(name: string): void {
...
function loadFromDisk(): void {
  try {
    ...
    if (parsed.last_crash_at) state.last_crash_at = parsed.last_crash_at;
    if (parsed.started_at) state.started_at = parsed.started_at;
  } catch {
    // ignore malformed
  }
}
...
let installed = false;
export function installMetrics(): void {
  if (installed) return;
  installed = true;
  loadFromDisk();
...
export function getMetricsSnapshot(): MetricsState {
  return JSON.parse(JSON.stringify(state)) as MetricsState;
}
