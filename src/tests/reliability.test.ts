...
test('receipt before ingest remains visible after ingest',()=>{
 ...
 const [m]=store.listMessages(instance,jid,10); assert.equal(m.delivery?.state,'delivered'); assert.equal(m.delivery?.statusCode,3);
});
...
test('batch delivery lookup is bounded to requested ids',()=>{
 ...
 assert.equal(states.size,2); assert.equal(states.get('m-b')?.state,'failed');
});
