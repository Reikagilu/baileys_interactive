...
router.patch('/:instance/chatwoot', (req: Request, res: Response) => {
  ...
  let ignoreJids: string[] | undefined;
  if (body.ignoreJids !== undefined) {
    if (Array.isArray(body.ignoreJids)) {
      ...
      ignoreJids = (body.ignoreJids as string)
        .split(/[\n,;]+/)
        .map((j) => j.trim())
        .filter(Boolean);
    }
  ...
  const integration = updateChatwootConfig(instance, {
    ...
    daysLimitImportMessages:
      body.daysLimitImportMessages !== undefined ? Number(body.daysLimitImportMessages) : undefined,
    ignoreJids,
 ...
 */
router.post('/:instance/chatwoot/webhook', async (req: Request, res: Response) => {
  const instance = getInstanceParam(req, res);
  if (!instance) return;
 ...
 */
router.post('/:instance/chatwoot/invalidate-cache', (req: Request, res: Response) => {
  const instance = getInstanceParam(req, res);
  if (!instance) return;
