...
function cleanupDeliveryHistory() {
...
function enqueueDelivery(webhook, event, payload, instance) {
    const now = Date.now();
...
export function retryWebhookDelivery(id) {
    ...
    db.prepare(`UPDATE webhook_deliveries
     SET status = 'pending',
         ...
         next_attempt_at = ?,
         updated_at = ?
     WHERE id = ?`).run(maxAttempts, now, now, id);
...
export function claimDueDeliveries(batchSize, workerId, lockMs) {
    ...
    try {
        const rows = db
            .prepare(`SELECT id FROM webhook_deliveries
         WHERE (
            status = 'pending'
...
export function markDeliverySuccess(id, responseStatus) {
    const now = Date.now();
    db.prepare(`UPDATE webhook_deliveries
     SET status = 'delivered', response_status = ?, delivered_at = ?, updated_at = ?
         , lock_owner = NULL, lock_expires_at = NULL
     WHERE id = ?`).run(responseStatus, now, now, id);
}
export function markDeliveryRetry(id, lastError, responseStatus, attemptCount) {
    ...
    db.prepare(`UPDATE webhook_deliveries
     SET status = 'pending',
         ...
         next_attempt_at = ?,
         updated_at = ?
     WHERE id = ?`).run(lastError, responseStatus, nextAttemptAt, now, id);
}
export function markDeliveryFailed(id, lastError, responseStatus) {
    const now = Date.now();
    db.prepare(`UPDATE webhook_deliveries
     SET status = 'failed',
         last_error = ?,
         ...
         lock_expires_at = NULL,
         updated_at = ?
     WHERE id = ?`).run(lastError, responseStatus, now, id);
}
