...
import { backfillContactsFromMessages, listContacts as msListContacts, } from '../services/message-store.js';
const router = Router();
 ...
 */
router.post('/:name/pairing-code', async (req, res) => {
 ...
 */
router.get('/:name/chats/:jid/messages', async (req, res) => {
 ...
 */
router.post('/:name/chats/:jid/messages', async (req, res) => {
    const { name, jid } = req.params;
    const ctx = getInstance(name);
    if (!ctx) {
        return sendError(res, 404, 'instance_not_found');
 ...
 */
router.post('/:name/chats/:jid/sync-history', async (req, res) => {
    const { name, jid } = req.params;
    const decodedJid = decodeURIComponent(jid);
    const body = (req.body ?? {});
 ...
 */
router.patch('/:name/settings/general', async (req, res) => {
    const { name } = req.params;
    ...
    const updated = updateInstanceGeneral(name, {
        ...
        importContacts: body.importContacts,
    });
    writeAuditEvent(req, res, {
 ...
 */
router.post('/:name/contacts/backfill', (req, res) => {
    const { name } = req.params;
    ...
    try {
        ...
        return sendError(res, 500, 'backfill_failed', err?.message);
    }
});
/**
 ...
 */
router.get('/:name/contacts', (req, res) => {
    const { name } = req.params;
    if (!isValidInstanceName(name)) {
        return sendError(res, 400, 'invalid_instance_name');
    }
    ...
    try {
        ...
        return sendError(res, 500, 'list_contacts_failed', err?.message);
    }
});
 ...
 */
router.patch('/:name/settings/proxy', (req, res) => {
    const { name } = req.params;
    const before = getInstancePanelConfig(name);
    const body = (req.body ?? {});
 ...
 */
router.patch('/:name/events', (req, res) => {
    const { name } = req.params;
    const body = (req.body ?? {});
 ...
 */
router.post('/:name/events/test', async (req, res) => {
    const { name } = req.params;
    const body = (req.body ?? {});
 ...
 */
router.post('/:name/logout', async (req, res) => {
    try {
        const { name } = req.params;
