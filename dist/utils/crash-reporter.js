...
function appendCrashLine(line) {
    try {
        ...
        try {
            process.stderr.write(`crash-reporter: falha ao escrever crash log: ${String(err)}\n`);
        }
        catch { }
    }
}
export function recordCrash(report) {
 ...
 */
export function emitCrashWebhook(report) {
    try {
        emitWebhookEvent('process.crash', report, undefined);
    }
    catch (err) {
        try {
            process.stderr.write(`crash-reporter: webhook emit failed: ${String(err)}\n`);
        }
        catch { }
    }
}
