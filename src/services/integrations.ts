...
export interface ChatwootConfig {
  ...
  /**
   * @deprecated Use `GeneralConfig.importContacts` (em instance-config.ts).
   * Mantido apenas para compatibilidade retroativa com payloads antigos da
   * API e instâncias migradas. A leitura efetiva (handlers Baileys e
   * chatwoot-bridge) acontece em `getInstanceGeneral().importContacts`.
   * `migrateLegacyImportContactsFlag()` copia o valor antigo para o General
   * uma única vez na inicialização do servidor.
   */
 ...
 */
export function migrateLegacyImportContactsFlag(): void {
  let migrated = 0;
  let scanned = 0;
  try {
    const rows = db
      .prepare('SELECT instance, chatwoot_json FROM integration_configs')
      .all() as Array<{ instance: string; chatwoot_json: string }>;
    for (const row of rows) {
      scanned += 1;
      try {
        const parsed = JSON.parse(row.chatwoot_json) as { importContacts?: unknown } | null;
        const legacyValue = parsed && typeof parsed === 'object' ? parsed.importContacts : undefined;
        if (legacyValue !== true) continue;
        ...
        log.app.warn(
          'failed to migrate legacy importContacts flag for instance',
          { instance: row.instance, err: (err as Error)?.message },
        );
      }
    }
  } catch (err) {
    log.app.warn(
      'failed to scan integration_configs for legacy importContacts migration',
  ...
  if (migrated > 0) {
    log.app.info(
      'migrated legacy chatwoot.importContacts → general.importContacts',
      { scanned, migrated },
    );
  }
}
